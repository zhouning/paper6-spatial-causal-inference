...
- `analysis_joined.csv` has 3044 rows and includes `gc_propensity_score`, `gc_balancing_weight`, `gc_arcgis_style_matching_weight`, and `gc_arcgis_style_calibrated_weight`.
...
| `arcgis_style_matching_grid.csv` | ArcGIS-style GPS matching grid search over exposure bins and propensity/exposure scale values, with the selected row marked. | Audit parity against ArcGIS matching behavior and benchmark selected parameters. |
...
5. Style map layers from fields such as `gc_exposure`, `gc_outcome`, `gc_propensity_score`, `gc_balancing_weight`, `gc_arcgis_style_matching_weight`, `gc_arcgis_style_calibrated_weight`, and target fields prefixed with `gc_target_`.
...
print(joined[["gc_unit_id", "gc_exposure", "gc_outcome", "gc_balancing_weight", "gc_arcgis_style_calibrated_weight"]].head())
...
- `analysis_joined.csv` includes `gc_propensity_score`, `gc_balancing_weight`, `gc_arcgis_style_matching_weight`, and `gc_arcgis_style_calibrated_weight`; missing scores should be explained in `gis_run_summary.json.warnings`.
