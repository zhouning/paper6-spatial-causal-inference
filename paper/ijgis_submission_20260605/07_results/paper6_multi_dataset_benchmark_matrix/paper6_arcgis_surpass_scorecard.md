...
- `direct_arcgis_real_dataset_coverage`: `insufficient_evidence` - Add additional real ArcGIS comparisons before claiming broad superiority.
...
| direct_arcgis_real_dataset_coverage   | evidence_coverage            | insufficient_evidence |    2           |           nan      | 3                                                             | all_arcgis_available_rows  | Only a small number of real datasets have direct ArcGIS built-in comparisons.                    | Add additional real ArcGIS comparisons before claiming broad superiority.                                   |
| direct_arcgis_calibrated_balance_wins | direct_arcgis_metric         | surpasses_arcgis      |    2           |             2      | GeoCausal calibrated balance lower on every direct ArcGIS row | all_arcgis_available_rows  | GeoCausal calibrated balance beats ArcGIS on every direct ArcGIS comparison row.                 | Keep adding real ArcGIS comparisons and preserve the calibrated-balance win rate.                           |
