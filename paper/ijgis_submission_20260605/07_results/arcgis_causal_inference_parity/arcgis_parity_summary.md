...
| ols_or_gradient_boosting_propensity_score | OLS propensity score by default, with gradient boosting fallback. | partial | P0 | GPS-style ERF and adjusted outcome workflows | Add ArcGIS-named score fields and later optional gradient boosting parity. |
...
| weighted_correlation_balance_threshold | Judge confounder balance with weighted correlation and a threshold. | partial | P0 | balance diagnostics and evidence-grade rules | Add ArcGIS-compatible mean, median, and max absolute-correlation fields. |
