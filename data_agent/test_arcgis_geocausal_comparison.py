...
def _write_open_gis_fixture(tmp_path: Path) -> Path:
    ...
    pd.DataFrame(
        {"exposure": [1.0, 2.0, 3.0], "response": [10.1, 12.0, 13.9]}
    ).to_csv(open_dir / "gis_arcgis_style_calibrated_erf_curve_200.csv", index=False)
    pd.DataFrame(
        {"exposure": [1.0, 2.0, 3.0], "response": [10.2, 12.1, 13.8]}
    ).to_csv(open_dir / "gis_preferred_erf_curve_200.csv", index=False)
    pd.DataFrame(
        [
...
def test_build_arcgis_geocausal_comparison_writes_metrics_and_report(tmp_path):
    ...
    assert manifest["metrics"]["geocausal_preferred_erf_rows"] == 3
    assert round(manifest["metrics"]["preferred_erf_response_mae"], 4) == 0.1667
    assert round(manifest["metrics"]["preferred_erf_response_rmse"], 4) == 0.1732
    assert manifest["metrics"]["geocausal_arcgis_style_calibrated_erf_rows"] == 3
    assert round(manifest["metrics"]["arcgis_style_calibrated_erf_response_mae"], 4) == 0.0667
    assert round(manifest["metrics"]["arcgis_style_calibrated_erf_response_rmse"], 4) == 0.0816
    ...
    assert rows["preferred_erf_response_mae"] == "computed"
    assert rows["arcgis_style_erf_response_mae"] == "computed"
    assert rows["arcgis_style_calibrated_erf_response_mae"] == "computed"
    ...
    assert "Preferred ERF response MAE" in report
    assert "ArcGIS-style ERF response MAE" in report
    assert "ArcGIS-style calibrated ERF response MAE" in report
